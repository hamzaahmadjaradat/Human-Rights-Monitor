...
from app.database import incident_collection, evidence_collection
...
def create_report_with_files(report_data: dict, files: Optional[List[UploadFile]] = None):
    for v in report_data["incident_details"]["violation_types"]:
        if v not in allowed_violations:
            raise HTTPException(status_code=400, detail=f"Invalid violation: {v}")
    ...
    result = incident_collection.insert_one(report_data)
    new_report = incident_collection.find_one({"_id": result.inserted_id})
    return serialize_report(new_report)

def list_reports(status: Optional[str] = None, date_from: Optional[str] = None, date_to: Optional[str] = None, city: Optional[str] = None):
    query = {}
    if status:
        query["status"] = status
    if city:
...
def update_status(report_id: str, status: str):
    if status not in allowed_statuses:
        raise HTTPException(status_code=400, detail="Invalid status value.")
    updated = incident_collection.update_one({"_id": ObjectId(report_id)}, {"$set": {"status": status}})
    if updated.modified_count == 0:
        raise HTTPException(status_code=404, detail="Report not found")
    report = incident_collection.find_one({"_id": ObjectId(report_id)})
    return serialize_report(report)

def analytics_by_violation():
    pipeline = [
        {"$unwind": "$incident_details.violation_types"},
        {"$group": {"_id": "$incident_details.violation_types", "count": {"$sum": 1}}}
    ]
    result = incident_collection.aggregate(pipeline)
    return {doc["_id"]: doc["count"] for doc in result}
